...
def process_video_task(self, task_data: Dict) -> Dict:
    ...
    try:
        ...
        if celebrity_detection or object_detection:
            ...
            # 4️⃣ Download pipeline result.json
            bucket = result_info["video_bucket"]
            key = result_info["result_key"]
            ...
            logger.info(f"Downloading pipeline result {key} → {celebrity_result_path}")
            s3.download_file(bucket, key, celebrity_result_path)
            ...
            if reference_image_path:
                ...
                while elapsed < max_wait_seconds:
                    r = requests.get(ref_status_url, timeout=10)
                    r.raise_for_status()
                    data = r.json()
                ...
                # 🔑 Re-download UPDATED result.json
