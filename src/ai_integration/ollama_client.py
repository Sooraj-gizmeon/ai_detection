...
class OllamaClient:
    ...
    async def _make_streamed_request(self, 
        ...
        # Only add format specification if explicitly requested
        if response_format:
    ...
    async def generate_comprehensive_analysis(self, transcription: Dict) -> Dict:
        ...
        # Use enhanced comprehensive analysis prompt when available
        if ENHANCED_OLLAMA_PROMPTS and 'comprehensive_analysis' in ENHANCED_OLLAMA_PROMPTS:
            self.logger.info(f"Using enhanced comprehensive analysis prompt")
            
            # Use optimized single-prompt approach
            prompt = ENHANCED_OLLAMA_PROMPTS['comprehensive_analysis'].format(
                transcription=json.dumps(transcription, indent=2)
            )
            ...
            if is_large_request:
                response = await self._make_streamed_request(
                    prompt=prompt,
                    model=self.get_best_model("analysis"),
                    cache_key=f"enhanced_comprehensive_{hash(str(transcription))}",
                    response_format="json"
                )
            else:
                response = await self._make_request(
            ...
            if 'segments' in result:
                return {
                    'content_engagement': {'engagement_analysis': result.get('segments', [])},
                    'analysis_method': 'enhanced_comprehensive',
                    'content_size': transcription_size
                }
            
            # If parsing didn't yield expected format, continue with standard approach
    ...
    async def _make_request(self, 
        ...
        # Only add format specification if explicitly requested (for structured responses)
        # Don't force JSON format for simple text generation like titles
        if response_format:
            request_data["format"] = response_format
            ...
            try:
                ...
                async with self.session.post(
                    ...
                    if response.status == 200:
                        ...
                        self.logger.debug(f"Raw API response structure: {type(result)} with keys: {list(result.keys()) if isinstance(result, dict) else 'Not a dict'}")
                        ...
                        # Log the response text type for debugging
                        self.logger.debug(f"Extracted response_text type: {type(response_text)}")
                        
                        # Ensure we always return a string
                        if isinstance(response_text, dict):
                            # If response_text is already a dict, convert to JSON string
                            response_text = json.dumps(response_text)
                            self.logger.debug(f"Converted dict response to JSON string")
                        elif not isinstance(response_text, str):
                            # If it's not a string or dict, convert to string
                            response_text = str(response_text)
                            self.logger.debug(f"Converted {type(response_text)} response to string")
    ...
    def _parse_json_response(self, response: Any) -> Dict:
        ...
        if isinstance(response, dict):
            # Check if this is a cached response with nested JSON data
            if 'metadata' in response and 'data' in response and 'data_type' in response.get('metadata', {}):
                self.logger.debug("Detected cached response with nested JSON data")
                try:
                    # Parse the nested JSON data
                    data_str = response['data']
                    if isinstance(data_str, str):
                        parsed_data = json.loads(data_str)
                        # Preserve analysis_type if present
                        if 'analysis_type' in response:
                            parsed_data['analysis_type'] = response['analysis_type']
                        self.logger.debug(f"Successfully parsed cached JSON data: {list(parsed_data.keys())}")
                        return parsed_data
                    else:
                        self.logger.warning(f"Cached data is not a string: {type(data_str)}")
                        return data_str if isinstance(data_str, dict) else response
                except json.JSONDecodeError as e:
                    self.logger.error(f"Failed to parse cached JSON data: {e}")
                    return response
            else:
                self.logger.debug("Response is already a dictionary, returning directly")
                return response
        
        # Handle case where response is not a string
        if not isinstance(response, str):
            self.logger.warning(f"Response is not a string or dict, got {type(response)}, converting to string")
    ...
    async def analyze_with_vision_context(self, transcription: Dict, vision_analysis: Dict) -> Dict:
        """
        Analyze content using both transcription and vision analysis for enhanced decision making.
        
        Args:
            transcription: Whisper transcription result
            vision_analysis: Vision analysis results from frames
            
        Returns:
            Combined dual-modal analysis
        """
        try:
            # Use enhanced dual-modal prompt if available
            if ENHANCED_OLLAMA_PROMPTS and 'dual_modal_analysis' in ENHANCED_OLLAMA_PROMPTS:
                prompt = ENHANCED_OLLAMA_PROMPTS['dual_modal_analysis'].format(
                    transcription=json.dumps(transcription, indent=2),
                    vision_analysis=json.dumps(vision_analysis, indent=2)
                )
                
                # Determine if this is a large request
                total_size = len(json.dumps(transcription)) + len(json.dumps(vision_analysis))
                is_large_request = total_size > 5000
                
                if is_large_request:
                    response = await self._make_streamed_request(
                        prompt=prompt,
                        model=self.get_best_model("vision"),
                        cache_key=f"dual_modal_{hash(str(transcription) + str(vision_analysis))}",
                        response_format="json"
                    )
                else:
                    response = await self._make_request(
                ...
                result = self._parse_json_response(response)
                result['analysis_type'] = 'dual_modal'
                result['input_size'] = total_size
                
                return result
            else:
                # Fallback to comprehensive analysis with vision context
                return await self.generate_comprehensive_analysis(transcription)
                
        except Exception as e:
            self.logger.error(f"Error in dual-modal analysis: {e}")
            return {
                'error': f'Dual-modal analysis failed: {str(e)}',
                'segments': [],
                'analysis_type': 'dual_modal_error'
            }
    
    async def analyze_visual_content(self, image_base64: str, analysis_type: str = "scene_analysis", audio_context: str = None) -> Dict:
        """
        Analyze visual content using vision models.
        
        Args:
            image_base64: Base64 encoded image
            analysis_type: Type of analysis (scene_analysis, engagement_assessment, content_context)
            audio_context: Optional audio context for enhanced analysis
            
        Returns:
            Visual analysis results
        """
        try:
            self.logger.debug(f"Starting visual content analysis: type={analysis_type}, image_size={len(image_base64)}")
            
            try:
                from config.ollama_prompts import VISION_ENHANCED_PROMPTS
                self.logger.debug(f"Successfully imported VISION_ENHANCED_PROMPTS with keys: {list(VISION_ENHANCED_PROMPTS.keys())}")
            except ImportError as e:
                self.logger.error(f"Failed to import VISION_ENHANCED_PROMPTS: {e}")
                # Fallback prompts
                VISION_ENHANCED_PROMPTS = {
                    'scene_analysis': """
                    Analyze this video frame briefly. Respond in JSON:
                    {{"scene_type": "talking_head", "people_count": 1, "visual_interest": 5, "description": "brief description"}}
                    """
                }
            
            # Get appropriate prompt
            if analysis_type == "content_context" and audio_context:
                prompt = VISION_ENHANCED_PROMPTS.get('content_context', VISION_ENHANCED_PROMPTS['scene_analysis']).format(audio_context=audio_context)
            elif analysis_type in VISION_ENHANCED_PROMPTS:
                prompt = VISION_ENHANCED_PROMPTS[analysis_type]
            else:
                # Default to scene analysis
                prompt = VISION_ENHANCED_PROMPTS.get('scene_analysis', 'Analyze this image briefly in JSON format.')
            
            self.logger.debug(f"Using prompt for {analysis_type}: {prompt[:100]}...")
            
            # Use vision model
            vision_model = self.get_best_model("vision")
            self.logger.debug(f"Using vision model: {vision_model}")
            
            response = await self._make_request(
                prompt=prompt,
                model=vision_model,
                cache_key=f"vision_{analysis_type}_{hash(image_base64[:100])}",
                images=[image_base64],
                response_format="json"
            )
            
            self.logger.debug(f"Raw response type: {type(response)}, content: {str(response)[:200]}...")
            
            result = self._parse_json_response(response)
            result['analysis_type'] = analysis_type
            
            # Enhanced logging for vision results
            if isinstance(result, dict):
                self.logger.info(f"Vision analysis result for {analysis_type}: {result}")
                self.logger.debug(f"Parsed result keys: {list(result.keys())}")
            else:
                self.logger.warning(f"Vision analysis returned non-dict result: {type(result)} - {result}")
    ...
    async def batch_analyze_frames(self, frames_data: List[Dict], analysis_type: str = "scene_analysis") -> List[Dict]:
        """
        Analyze multiple frames in batches for efficiency.
        
        Args:
            frames_data: List of frame dictionaries with base64 image data
        ...
        results = []
        batch_size = 3  # Process 3 frames at once to avoid overwhelming the API
        
        try:
            for i in range(0, len(frames_data), batch_size):
                batch = frames_data[i:i + batch_size]
                
                self.logger.info(f"Processing batch {i//batch_size + 1}: {len(batch)} frames")
                
                # Process batch in parallel
                batch_tasks = []
                for frame_data in batch:
                    if 'base64' in frame_data:
                        task = self.analyze_visual_content(
                            frame_data['base64'], 
                            analysis_type
                        )
                ...
                if batch_tasks:
                    self.logger.info(f"Executing {len(batch_tasks)} vision analysis tasks for batch")
                    batch_results = await asyncio.gather(*batch_tasks, return_exceptions=True)
                    
                    self.logger.info(f"Batch analyzed {len(batch)} frames, got {len(batch_results)} results")
                    ...
                    for j, result in enumerate(batch_results):
                        if isinstance(result, Exception):
                            ...
                            # Enhanced logging for vision results
                            if isinstance(result, dict):
                                self.logger.info(f"Frame {j} vision analysis: {result}")
                            else:
                                self.logger.warning(f"Frame {j} analysis returned non-dict: {type(result)} - {result}")
                            
                            # Add timestamp from original frame data
                            if isinstance(result, dict) and j < len(batch):
                                result['timestamp'] = batch[j].get('timestamp', 0)
                            results.append(result)
                    
                    # Brief pause between batches
                    if i + batch_size < len(frames_data):
                        await asyncio.sleep(0.5)
            
            self.logger.info(f"Batch analyzed {len(frames_data)} frames, got {len(results)} results")
            
            # Log summary of vision analysis results
            if results:
                ...
                scene_types = [r.get('scene_type', 'unknown') for r in results if isinstance(r, dict)]
                self.logger.info(f"Vision analysis summary: {success_count} successful, {error_count} errors")
                self.logger.info(f"Scene types detected: {dict(zip(*[list(set(scene_types)), [scene_types.count(x) for x in set(scene_types)]]))}")
