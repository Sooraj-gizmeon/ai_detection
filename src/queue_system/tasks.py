...
def process_video_task(self, task_data: Dict) -> Dict:
    ...
    try:
        ...
        # Local face/object detection pipeline integration
        if celebrity_detection or object_detection:
            try:
                # Import here to avoid circular import issues
                from src.face_detection.pipeline import run_pipeline
                logger.info("Running local face/object detection pipeline...")
                # Set output path for results
                job_id = self.request.id
                celebrity_result_path = f"output/celebrity_results/{job_id}.json"
                os.makedirs(os.path.dirname(celebrity_result_path), exist_ok=True)
                # Run the detection pipeline
                pipeline_result = run_pipeline(
                    video_path=input_path,
                    fps=1,
                    output_json=celebrity_result_path,
                    raw_dir="frames/raw",
                    unique_faces_dir="frames/unique",
                    unique_objects_dir="frames/unique_objects",
                ...
                if reference_image_path:
                    ...
                    logger.info("Running local reference image search...")
                    reference_search_result = search_reference_image(
                        reference_image_path=reference_image_path,
                        video_bucket=storage_bucket,
                        result_json_path=celebrity_result_path,
                ...
                logger.error(f"Local face/object detection pipeline failed: {e}")
        #     bypass_ada_server = os.getenv('BYPASS_ADA_SERVER', 'false').lower() in ('true', '1', 'yes')
        #     ...
        #     # All Ada server code commented out for local integration
        ...
        # This ensures proper processing order: extract segment → apply intro/outro → apply smart zoom/templates
        ...
        if 'shorts_details' in result and result['shorts_details']:
            ...
            # 🎬 LINEAR CUT OPTIMIZATION: Generate title, description, and tags once for all segments
            ...
            if linear_cut and result['shorts_details']:
                ...
                logger.info(f"🔍 DEBUG: Checking first_short for existing title")
                ...
                logger.info(f"🔍 Title validation: base_title='{base_title}', is_valid={is_valid_title}")
                ...
                if is_valid_title:
                    ...
                    logger.info("🎬 LINEAR CUT MODE: Using existing title from main processor")
                    ...
                    logger.info(f"🎬 LINEAR CUT MODE: Invalid/placeholder title detected ('{base_title}'), generating new title")
            ...
            for short in result['shorts_details']:
                try:
                    ...
                    try:
                        ...
                        # 🎬 LINEAR CUT MODE: Use pre-generated title, description, and tags with part numbers
                        if linear_cut and linear_cut_title and linear_cut_description:
                            ...
                            logger.info(f"🎬 LINEAR CUT: Using optimized title for part {part_number}: {ai_title}")
                            logger.info(f"🎬 LINEAR CUT: Using optimized tags: {tags}")
                        else:
                        ...
                        if linear_cut and is_last_clip:
                            ...
                            logger.info(f"📍 Last clip in linear cut mode - adding status='completed' to API request body")
...
#     #         logger.info(f"[task-{self.request.id}] Downloading → {local_fname}")
