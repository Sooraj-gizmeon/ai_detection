"""
Improved MediaPipe Face Detection with timestamp error fixes.
"""
...
class MediaPipeFaceDetector:
    """
    Face detection using MediaPipe with timestamp error fixes.
    """
    
    def __init__(self, device: str = "cuda"):
        """
        Initialize face detector without persistent instances.
    ...
    def detect_faces(self, frame: np.ndarray) -> List[Dict]:
        """
        Detect faces in frame with complete timestamp isolation.
        ...
        try:
            ...
            # Make frame contiguous and copy to avoid MediaPipe issues
            rgb_frame = np.ascontiguousarray(rgb_frame.copy())
            
            # Create a fresh detector instance for each frame to avoid timestamp issues
            # This is the most reliable way to prevent MediaPipe timestamp errors
            ...
            if results and results.detections:
                ...
                for detection in results.detections:
                    try:
                        ...
                        self.logger.debug(f"Failed to process face detection: {e}")
                        continue
            
            return faces
            
        except Exception as e:
            self.logger.debug(f"Face detection failed: {e}")
            return []
